...
def getTrainingSet(csvPath):
    ...
    trainingRowMask = '["*text*",{"entities":[[*corporation_start*,*corporation_end*,"corporation"],[*file_date_start*,*file_date_end*,"file_date"]]}]'
    trainingSet = []
    for row in taggedData:
        ...
        tempRow = trainingRowMask.replace('*text*',str(row[0])).replace('*corporation_start*',str(row[2])).replace('*corporation_end*',str(row[3])).replace('*file_date_start*',str(row[8])).replace('*file_date_end*',str(row[9]))
        #print(str(row[0]))
